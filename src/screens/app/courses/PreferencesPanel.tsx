import { Check, Clock, Coffee, Flame, Thermometer, Wallet, X } from 'lucide-react'
import { useApp } from '../../../context/AppContext'
import { Button } from '../../../components/ui'
import type { HotColdPattern, SnackTiming, TimeBand } from '../../../types'
...
const HOT_COLD_OPTIONS: { value: HotColdPattern | null; label: string }[] = [
  { value: null, label: 'Peu importe' },
  { value: 'chaud_chaud', label: 'Chaud le midi et le soir' },
  { value: 'froid_chaud', label: 'Froid le midi, chaud le soir' },
  { value: 'chaud_froid', label: 'Chaud le midi, froid le soir' },
  { value: 'froid_froid', label: 'Froid le midi et le soir' },
]
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-5 py-5 flex flex-col gap-7">
        ...
        <section>
          ...
          <div className="flex flex-col gap-2">
            {HOT_COLD_OPTIONS.map((opt) => (
              <button
                key={opt.label}
                onClick={() => setConstraints({ hotColdPattern: opt.value })}
                className={`tap flex items-center justify-between px-4 py-2.5 rounded-2xl border text-left text-[13px] font-semibold ${
                  constraints.hotColdPattern === opt.value ? 'border-leaf-500 bg-leaf-50 text-leaf-700' : 'border-black/10 bg-white text-ink-soft'
                }`}
              >
                {opt.label}
                {constraints.hotColdPattern === opt.value && <Check size={15} className="text-leaf-600 shrink-0" />}
              </button>
            ))}
          </div>
        </section>
