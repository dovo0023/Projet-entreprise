...
## Navigation & foyer (nouveau)
- [x] ~~L'onglet "Aujourd'hui" était en 2ᵉ position et il n'existait aucune notion de foyer~~ → résolu : la barre de navigation est réordonnée pour centrer "Aujourd'hui" (Planning, Courses, **Aujourd'hui**, Progression, Foyer), et un nouvel onglet **Foyer** permet de déclarer les autres personnes du domicile qui partagent les repas (prénom, objectif — sèche/maintien/prise de masse — et allergies propres à chacune). L'ancien réglage "Mode Duo / Famille" dans Profil a été retiré au profit de ce nouvel onglet (lien "Mon foyer" depuis Profil).
- [ ] Le foyer n'influence pour l'instant que l'affichage ("x{n} portions" dans Aujourd'hui) : le moteur de génération de menu ne recalcule pas encore les quantités/courses en fonction des objectifs et allergies de chaque membre — à faire si on va plus loin que la démo.
